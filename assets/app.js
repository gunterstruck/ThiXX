document.addEventListener('DOMContentLoaded', () => {
    ...
    const CONFIG = {
        ...
        MAX_WRITE_RETRIES: 3,
        // NEU: Die URL wird dynamisch erzeugt, um die App portabel zu machen.
    ...
    // --- Data Mapping ---
    // Maps form field names to short URL parameter keys
    const fieldMap = {
        ...
        'PT 100': 'PT100', 'NiCr-Ni': 'NiCr', 'geprüft von': 'Chk', 'am': 'Date', 'Dokumentation': 'Doc'
    };
    ...
    async function main() {
        ...
        // NEU: Verarbeite URL-Parameter beim Start.
    ...
    async function displayParsedData(data) {
        ...
        const addPair = (frag, labelKey, val, unit) => { const el = createDataPair(t(labelKey), val, unit); if (el) frag.appendChild(el); };

        addPair(fragments.main, 'form.HK.Nr.', data['HK.Nr.']);
        addPair(fragments.main, 'form.KKS', data['KKS']);
        addPair(fragments.section1, 'form.Leistung', data['Leistung'], 'kW');
        addPair(fragments.section1, 'form.Strom', data['Strom'], 'A');
        addPair(fragments.section1, 'form.Spannung', data['Spannung'], 'V');
        addPair(fragments.section1, 'form.Widerstand', data['Widerstand'], 'Ω');
        addPair(fragments.section2, 'form.Anzahl Heizkabeleinheiten', data['Anzahl Heizkabeleinheiten'], 'Stk');
        addPair(fragments.section2, 'form.Trennkasten', data['Trennkasten'], 'Stk');
        addPair(fragments.section2, 'form.Heizkabeltyp', data['Heizkabeltyp']);
        addPair(fragments.section2, 'form.Schaltung', data['Schaltung']);
        if (data['PT 100']) addPair(fragments.section2, 'form.Messwertgeber', `PT 100: ${data['PT 100']}`, 'Stk');
        if (data['NiCr-Ni']) addPair(fragments.section2, 'form.Messwertgeber', `NiCr-Ni: ${data['NiCr-Ni']}`, 'Stk');
        addPair(fragments.section3, 'form.Regler', data['Regler'], '°C');
        addPair(fragments.section3, 'form.Sicherheitsregler/Begrenzer', data['Sicherheitsregler/Begrenzer'], '°C');
        addPair(fragments.section3, 'form.Wächter', data['Wächter'], '°C');
        addPair(fragments.footer, 'form.Projekt Nr.', data['Projekt Nr.']);
        addPair(fragments.footer, 'form.geprüft von', data['geprüft von']);
        addPair(fragments.footer, 'form.am', data['am']);
    ...
    async function handleNfcAction() {
        ...
        // GEÄNDERT: Aktives Lesen ist nicht mehr nötig. Button gibt nur noch eine Info aus.
        if (!isWriteMode) {
        ...
        try {
            ...
            setNfcBadge('writing');
            // NEU: Schreibe die generierte URL anstatt des v1-Textformats.
    ...
    /**
     * NEU: Verarbeitet URL-Parameter beim Laden der App.
     * @returns {boolean} - True, wenn Daten verarbeitet wurden, sonst false.
     */
    function processUrlParameters() {
        ...
        if (Object.keys(data).length > 0) {
            ...
            // Bereinigt die URL, damit die Daten bei einem Refresh nicht erneut geladen werden.
    ...
    /**
     * NEU: Erstellt die vollständige URL mit allen Formulardaten als Query-Parameter.
    ...
    function loadJsonIntoForm(event) {
        const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (e) => {
            try {
                const data = JSON.parse(e.target.result); appState.scannedDataObject = data; populateFormFromScan(); showMessage(t('messages.loadSuccess'), 'ok')
            } catch (error) {
                const userMessage = error instanceof SyntaxError ? 'Die JSON-Datei hat ein ungültiges Format.' : error.message;
                ErrorHandler.handle(new Error(userMessage), 'LoadJSON');
