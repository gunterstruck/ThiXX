document.addEventListener('DOMContentLoaded', () => {
    ...
    // KORREKTUR (PRIO 2): Hilfsfunktion zur Erkennung von iOS.
    ...
    async function main() {
            ...
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('/ThiXX/sw.js', { scope: '/ThiXX/' })
                    .then(registration => {
                        ...
                        // VERBESSERUNG (PRIO 4): Auf SW-Updates prüfen und UI anzeigen.
        ...
        checkNfcSupport();
        initCollapsibles();
        if (!processUrlParameters()) { setupReadTabInitialState(); switchTab('read-tab'); }
    }
    main();
    ...
    async function displayParsedData(data) { protocolCard.innerHTML = ''; const fragments = { main: document.createDocumentFragment(), section1: document.createDocumentFragment(), section2: document.createDocumentFragment(), section3: document.createDocumentFragment(), footer: document.createDocumentFragment() }; const addPair = (frag, labelKey, val, unit) => { const el = createDataPair(t(labelKey), val, unit); if (el) frag.appendChild(el); }; addPair(fragments.main, 'form.HK.Nr.', data['HK.Nr.']); addPair(fragments.main, 'form.KKS', data['KKS']); addPair(fragments.section1, 'form.Leistung', data['Leistung'], 'kW'); addPair(fragments.section1, 'form.Strom', data['Strom'], 'A'); addPair(fragments.section1, 'form.Spannung', data['Spannung'], 'V'); addPair(fragments.section1, 'form.Widerstand', data['Widerstand'], 'Ω'); addPair(fragments.section2, 'form.Anzahl Heizkabeleinheiten', data['Anzahl Heizkabeleinheiten'], 'Stk'); addPair(fragments.section2, 'form.Trennkasten', data['Trennkasten'], 'Stk'); addPair(fragments.section2, 'form.Heizkabeltyp', data['Heizkabeltyp']); addPair(fragments.section2, 'form.Schaltung', data['Schaltung']); if (data['PT 100']) addPair(fragments.section2, 'form.Messwertgeber', `PT 100: ${data['PT 100']}`, 'Stk'); if (data['NiCr-Ni']) addPair(fragments.section2, 'form.Messwertgeber', `NiCr-Ni: ${data['NiCr-Ni']}`, 'Stk'); addPair(fragments.section3, 'form.Regler', data['Regler'], '°C'); addPair(fragments.section3, 'form.Sicherheitsregler/Begrenzer', data['Sicherheitsregler/Begrenzer'], '°C'); addPair(fragments.section3, 'form.Wächter', data['Wächter'], '°C'); addPair(fragments.footer, 'form.Projekt Nr.', data['Projekt Nr.']); addPair(fragments.footer, 'form.geprüft von', data['geprüft von']); addPair(fragments.footer, 'form.am', data['am']); const createSection = (frag, className) => { if (frag.hasChildNodes()) { const section = document.createElement('div'); section.className = className; section.appendChild(frag); protocolCard.appendChild(section); } }; createSection(fragments.main, 'card-main'); createSection(fragments.section1, 'card-section'); createSection(fragments.section2, 'card-section'); createSection(fragments.section3, 'card-section'); createSection(fragments.footer, 'card-footer'); docLinkContainer.innerHTML = ''; if (data['Dokumentation']) { const url = data['Dokumentation']; if (!isValidDocUrl(url)) { console.warn('Invalid documentation URL provided:', url); return; } const button = document.createElement('button'); button.className = 'btn doc-link-btn'; button.dataset.url = url; const isCached = await isUrlCached(url); if (isCached) { button.textContent = t('docOpenOffline'); button.onclick = () => window.open(url, '_blank'); } else { button.textContent = navigator.onLine ? t('docDownload') : t('docDownloadLater'); button.addEventListener('click', handleDocButtonClick); } docLinkContainer.appendChild(button); } }

    function applyConfig(config) {
        const selectedDesign = designs[config.design] || designs['thixx_standard'];
        // KORREKTUR (PRIO 2): Dynamisches Manifest nur für Nicht-iOS-Geräte erstellen.
        if (!isIOS()) {
    ...
    function processUrlParameters() { const params = new URLSearchParams(window.location.search); if (params.toString() === '') return false; const data = {}; for (const [shortKey, value] of params.entries()) { const fullKey = reverseFieldMap[shortKey]; if (fullKey) data[fullKey] = decodeURIComponent(value); } if (Object.keys(data).length > 0) { appState.scannedDataObject = data; displayParsedData(data); rawDataOutput.value = window.location.href; readActions.classList.remove('hidden'); readResultContainer.classList.add('expanded'); switchTab('read-tab'); history.replaceState(null, '', window.location.pathname); showMessage(t('messages.readSuccess'), 'ok'); return true; } return false; }
    ...
    // --- UI/UX Functions ---
    function updateManifest(design) { const manifestLink = document.querySelector('link[rel="manifest"]'); if (!manifestLink) return; const oldHref = manifestLink.href; if (oldHref && oldHref.startsWith('blob:')) { URL.revokeObjectURL(oldHref); } const newManifest = { name: design.appName, short_name: design.appName.split(' ')[0], start_url: "/ThiXX/index.html", scope: "/ThiXX/", display: "standalone", background_color: "#ffffff", theme_color: design.brandColors.primary || "#f04e37", orientation: "portrait-primary", icons: [{ src: design.icons.icon192, sizes: "192x192", type: "image/png" }, { src: design.icons.icon512, sizes: "512x512", type: "image/png" }] }; const blob = new Blob([JSON.stringify(newManifest)], { type: 'application/json' }); manifestLink.href = URL.createObjectURL(blob); }
